    ...
    if (tick) {
      ...
      const fetchData = async () => {
        try {
          ...
          const statsResponse = await axios.get(`https://tn11api.kasplex.org/v1/krc20/token/${tick}?stat=true`);
            ...
            {holder.map((holder, index) => (
              <tr key={index} style={{ transition: 'background-color 0.3s ease', backgroundColor: index % 2 === 0 ? '#2b2b2b' : '#1f1f1f' }}>
                <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                  <a href={`https://explorer-tn11.kaspa.org/addresses/${holder.address}?page=1`} target="_blank" rel="noopener noreferrer" style={{ color: 'white', textDecoration: 'none' }}>
